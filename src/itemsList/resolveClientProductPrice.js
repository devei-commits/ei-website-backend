...
async function resolveClientProductPrice({ productId, clientId, quantity = 1 }) {
  ...
  return {
    ...
    source: tier ? 'client_price_list_tier' : price === mrpReference ? 'mrp_reference' : 'client_price_list_rate',
    items_list_id: listRow.id,
    ...
    message:
      tier
        ? `Price from client price list (MOQ tier from ${moqMin}${moqMax != null ? `–${moqMax}` : '+'})`
        : price === mrpReference
          ? 'No matching MOQ tier — using MRP reference or default rate.'
          : 'Price from client price list default rate.',
  };
}
