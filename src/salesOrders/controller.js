...
function bodyToPayload(body) {
  ...
  return {
    order_id: body.orderId ?? formData.orderId ?? '',
    customer_name: body.customerName ?? formData.customerName ?? null,
...
function bodyToUpdatePayload(body) {
  ...
  const formData = body.formData && typeof body.formData === 'object' ? body.formData : {};
  const payload = {};
  if (body.orderId !== undefined || formData.orderId !== undefined) payload.order_id = body.orderId ?? formData.orderId ?? '';
  if (body.customerName !== undefined || formData.customerName !== undefined) payload.customer_name = body.customerName ?? formData.customerName ?? null;
