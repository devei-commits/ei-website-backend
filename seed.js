...
async function seed() {
  try {
    ...
    await BOM.bulkCreate([
      ...
      // Facewash 150ml — Pack BOM (and RM) for EI-SO-2026-001 reservations
      {
    ...
    const planSOs = await SalesOrder.findAll({ where: { order_id: 'EI-SO-2026-001' }, order: [['order_id']] });
    const planProds = await Product.findAll({ where: { product_code: 'EI-PR-00002' }, order: [['product_code']] });
    ...
    if (planSOs.length >= 1 && planProds.length >= 1) {
      // Facewash — 12 RM, 4 PM; BOM confirmed, 15 batches planned
      const facewashRm = [
